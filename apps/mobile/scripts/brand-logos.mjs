 ...
 * fetches from Wikimedia. The list is written to src/lib/brandLogos.ts, and
 * the images, which only the bundler can load, to src/lib/brandLogoImages.ts.
 *
...
const outImages = join(here, '..', 'src', 'lib', 'brandLogoImages.ts');
...
async function main() {
  ...
  const lines = [
    ...
    ...logos.map(
      (logo) =>
        `  { qid: ${JSON.stringify(logo.qid)}, brand: ${JSON.stringify(logo.brand)}, slug: ${JSON.stringify(logo.slug)}, aspect: ${logo.aspect}, page: ${JSON.stringify(
  ...
  writeFileSync(
    ...
    [
      ...
      ...logos.map((logo) => `  ${JSON.stringify(logo.slug)}: require('../../assets/logos/${logo.slug}.png'),`),
      '};',
      '',
    ].join('\n'),
  );
