...
describe('opening a database made by an older GymGO', () => {
  it('widens visit prices to euros, pounds and Swiss francs, keeping every report', () => {
    ...
    expect(() => db.prepare("insert into price_reports values ('a-tokyo-gym', 'u1', 1500, 'JPY', '2026-09-01', '2026-09-02T00:00:00Z')").run()).toThrow();
    db.close();
    // Opening it again changes nothing.
    const again = openDb(path);
    expect((again.prepare('select count(*) as n from price_reports').get() as { n: number }).n).toBe(2);
    again.close();
  });
});
