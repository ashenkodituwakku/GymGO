...
| Training log: start a built or saved workout, tick sets (last time's numbers as placeholders), rest timer, plates per side (lb and kg, drawn side on: the collar, then each plate to size; the bare bar and what will show before you type), records on finishing (heaviest, estimated 1RM, body-weight reps; only against earlier sessions), Progress (week streak, this week, records, history grouped by day with each session's start time, delete); in progress kept on the device; saved to the account (free); lb in the US, kg elsewhere | ✅ | ✅ 24 unit tests (records, targets, plates, streak, days, parsing) + 6 server tests (auth, own sessions only, validation, export, deleted with the account) + driven in the browser: two sessions logged, a reload mid-workout kept both sets, the second broke a record, Progress and plates checked, Free and Pro views | n/a (your own numbers) | ❌ | n/a | ❌ |
...
| App icon (iOS light, dark and tinted; Android adaptive and themed), splash screen, favicon | ✅ | ⚠️ made by `apps/mobile/scripts/brand-mark.mjs`; the favicon seen in the browser, the icons never on a phone's home screen | n/a | ❌ | n/a | ❌ |
...
| A place GymGO carries no city for (the capital of a country you choose, like Tokyo for Japan) reads the map around it by itself, once per block of tiles, instead of opening empty: "Looking for gyms around Tokyo…", then the gyms; if the map service fails, says so with Try again (never retried on its own); when nothing is in range, says that instead of blaming the filters | ✅ | ✅ 5 unit tests + driven in the browser against a local stand-in Overpass (the real servers are blocked from this sandbox): Japan (one request, gyms on Home and Explore), Kenya while slow, Peru failing then Try again, phone / tablet / desktop, light and dark | ✅ Overpass, via the area search (real servers not reached from here) | ❌ | n/a | ❌ |
...
| `@gymgo/mobile` unit tests | **150 passed** |
