...
| Search this area: gyms from OpenStreetMap for wherever the map is (AU and US), same rules as the bundled cities, kept a month per 11 km tile | ✅ | ✅ 13 server tests (stand-in Overpass, incl. falling over to the next server and today’s rules applied to kept gyms) + 4 unit tests + driven in the browser over Bendigo (7 gyms found live) and Hobart, desktop and phone layouts; the Android map page's area reporting checked in Chromium with a stand-in app bridge (reports the area above the sheet) | ✅ Overpass API, read live (a mirror from this sandbox; the main server refuses it) | ❌ | n/a | ❌ |
| Opening outside the carried cities (in AU or the US) searches the map around you and shows "Near you"; elsewhere, the nearest carried city as before | ✅ | ✅ 2 unit tests + driven in the browser with a location in Bendigo (7 gyms) and Auckland (Sydney, 2,156 km) | ✅ Overpass, via the area search | ❌ | n/a | ❌ |
